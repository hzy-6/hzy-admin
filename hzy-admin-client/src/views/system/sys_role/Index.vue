...
async function findList() {
  state.loading = true;
  const result = await SysRoleService.findList(state.page, state.size, state.search.vm);
  state.loading = false;
...
function exportExcel() {
  SysRoleService.exportExcel(state.search.vm);
}
</script>
  ...
  <PageContainer>
    ...
    >
      ...
      <template #table-col>
        ...
        <a-table-column title="数据权限" data-index="permissionType">
        ...
        <a-table-column title="删除锁定" data-index="deleteLock">
          <template #default="{ record }">
