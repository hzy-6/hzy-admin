  ...
  <div class="p-20">
    <CrudList ref="refCrudList" :tableData="state" @onCurrentChange="() => findList()" @onSizeChange="() => findList()">
      ...
      <template #table-col>
        ...
        <el-table-column prop="permissionType" label="数据权限">
          <template #default="scope">
            <el-tag type="success" v-if="scope.row.permissionType === 1">自定义权限</el-tag>
            <el-tag type="success" v-if="scope.row.permissionType === 2">查看所有数据</el-tag>
            <el-tag type="success" v-if="scope.row.permissionType === 3">仅查看本组织</el-tag>
            <el-tag type="success" v-if="scope.row.permissionType === 4">仅查看本组织和下属组织</el-tag>
            <el-tag type="success" v-if="scope.row.permissionType === 5">仅查看本人</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="deleteLock" label="删除锁定">
          <template #default="scope">
            <el-tag v-if="scope.row.deleteLock">锁定</el-tag>
            <el-tag v-else type="warning">不锁定</el-tag>
          </template>
        </el-table-column>
